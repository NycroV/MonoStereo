    ...
    class Codebook : ICodebook
    {
        ...
        private void InitTree(IPacket packet, IHuffman huffman)
        {
            ...
            if (packet.ReadBit())
            {
                ...
                {
                    var cnt = (int)packet.ReadBits(NVorbisUtils.ilog(Entries - i));
        ...
        bool ComputeCodewords(bool sparse, int[] codewords, int[] codewordLengths, int[] len, int n, int[] values)
        {
            ...
            for (i = k + 1; i < n; ++i)
            {
                ...
                res = available[z];
                available[z] = 0;
                AddEntry(sparse, codewords, codewordLengths, NVorbisUtils.BitReverse(res), i, m++, len[i], values);
        ...
        private void InitLookupTable(IPacket packet)
        {
            ...
            var minValue = NVorbisUtils.ConvertFromVorbisFloat32((uint)packet.ReadBits(32));
            var deltaValue = NVorbisUtils.ConvertFromVorbisFloat32((uint)packet.ReadBits(32));
